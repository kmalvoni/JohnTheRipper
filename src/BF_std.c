...
/* FPGA code added by Katja Malvoni*/
...
#pragma omp parallel for default(none) private(t) shared(n, BF_init_state, BF_init_key, BF_exp_key, salt, BF_magic_w, BF_out)
	...
	for_each_t() {
		...
		memcpy(src.S, BF_current INDEX2.S, sizeof(src.S));
		memcpy(src.P, BF_current INDEX2.P, sizeof(src.P));
		memcpy(src.exp_key, BF_exp_key INDEX2, sizeof(src.exp_key));
	...
	//	}
		...
		memcpy(BF_current INDEX2.S, dest.S, sizeof(src.S));
		memcpy(BF_current INDEX2.P, dest.P, sizeof(src.P));
		memcpy(BF_exp_key INDEX2, dest.exp_key, sizeof(src.exp_key));
